...
class ControllerSaleOrder extends Controller {
	...
	public function info() {
		...
		if ($order_info) {
			...
			// -webusis- не понятные сопоставления но нужен custom_field 
			foreach ($custom_fields as $custom_field) {
				if ($custom_field['location'] == 'shipping' && isset($order_info['shipping_custom_field'][$custom_field['custom_field_id']])) {
					if ($custom_field['type'] == 'select' || $custom_field['type'] == 'radio') {
					...
					// -webusis- захардил для звонка. Связь плохо работает с чекбоксом в retailcrm
					if ($custom_field['type'] == 'checkbox') {
						$data['shipping_custom_fields'][] = array(
							'name'  => $custom_field['name'],
							'value' => (!empty($order_info['shipping_custom_field'][$custom_field['custom_field_id']])) ? 'Yes' : 'No',
							'sort_order' => $custom_field['sort_order']
						);
					}
